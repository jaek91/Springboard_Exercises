...
console.log(onlyEvenValues([1,2,3,4,4]));
/*
...
function showFirstAndLast(arr){
    let result = [];
    arr.forEach(function(el) {
        let newString = el.slice(0,1) + el.slice(-1);
...
function vowelCount(str){
    ...
    let strArray = str.split("");
     ...
     strArray.forEach(function(str) {
        let lowerCaseLetters = str.toLowerCase();
        let indices = vowels.indexOf(lowerCaseLetters);
        if (indices !== -1){
            if (vowelsObject[lowerCaseLetters]){
                vowelsObject[lowerCaseLetters]++;
            }
            else {
                vowelsObject[lowerCaseLetters] = 1;
            }
        }
    })
     return vowelsObject; 
...
function removeVowels(str) {
    let lowercaseLetters = str.toLowerCase();
    const vowels ="aeiou";
    let strArray = Array.from(lowercaseLetters);
    return strArray.filter(function(el) {
...
console.log(doubleOddNumbers([1,3,5,7,4]));